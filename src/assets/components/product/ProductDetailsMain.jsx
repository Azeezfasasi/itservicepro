...
const ProductDetailsMain = () => {
  ...
  const incrementQuantity = () => {
    if (quantity < (product?.countInStock || 0)) {
          ...
          <div className="space-y-4">
            <div className="aspect-w-1 aspect-h-1 bg-gray-200 rounded-lg overflow-hidden">
              {product.images && product.images.length > 0 ? (
                <img
                  src={product.images[selectedImage].startsWith('http') ? product.images[selectedImage] : `${process.env.REACT_APP_API_URL}${product.images[selectedImage]}`}
                  alt={product.name}
                ...
                {(product.dimensions.length || product.dimensions.width || product.dimensions.height) && (
                  <>
                    ...
                    <dd className="text-sm text-gray-900">
                      {product.dimensions.length ? product.dimensions.length : '0'} × {product.dimensions.width ? product.dimensions.width : '0'} × {product.dimensions.height ? product.dimensions.height : '0'} cm
                    </dd>
                  </>
                )}
              </dl>
