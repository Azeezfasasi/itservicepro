...
//     if (!cart || !cart.items || cart.items.length === 0) { // Added !cart.items for robustness
//       errors.cart = 'Your cart is empty. Please add items to proceed.';
//     }

//     // Check for null or invalid product IDs in cart items
//     if (cart && cart.items && cart.items.some(item => !item.productId || (typeof item.productId === 'object' && !item.productId._id))) {
//       errors.cartItems = 'Some items in your cart have invalid product data. Please review your cart.';
//     }
...
//         // Even more robust productId extraction
//         productId: (typeof item.productId === 'object' && item.productId !== null)
//                    ? item.productId._id // If it's an object, get its _id
//                    : item.productId,    // Otherwise, assume it's already the ID string (or null/undefined, caught by backend validation)
//         name: item.name,
...
//             {cart.items.map(item => (
//               <div key={item.productId?._id || item.productId} className="flex items-center justify-between text-sm text-gray-700">
...
// frontend/src/components/checkout/CheckoutMain.jsx
    ...
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      ...
      <form onSubmit={handleSubmitCheckout} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 bg-white shadow-lg rounded-lg p-6 h-fit sticky top-4">
          ...
          <div className="space-y-4 mb-6 max-h-60 overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100">
            {cart.items.map((item, index) => { // Added index for fallback key
                // Determine a unique and stable key. Prefer productId, fallback to name+index.
                // This ensures React's key warning is resolved, but doesn't fix the underlying data.
                const itemKey = (typeof item.productId === 'string' && item.productId)
                                ? item.productId
                                : (typeof item.productId === 'object' && item.productId !== null && item.productId._id)
                                    ? item.productId._id
                                    : `${item.name || 'unknown'}-${index}`; // Fallback to a unique string
          ...
          <button
            type="submit"
            disabled={isLoading || !cart || cart.items.length === 0 || Object.keys(validationErrors).length > 0} // Disable if validation errors exist
            className="w-full mt-8 bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-6 rounded-md text-center transition duration-300 ease-in-out transform hover:scale-105 flex items-center justify-center gap-2
                       disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isLoading ? (
