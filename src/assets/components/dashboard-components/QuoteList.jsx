...
import React, { useState, useEffect } from 'react';
import { useQuote } from '../../context-api/Request-quote-context/UseQuote';
import { FaEdit, FaTrash, FaCheck, FaTimes, FaEye, FaFilter, FaSearch, FaSave, FaTimesCircle, FaSortAmountDown, FaSortAmountUp, FaReply } from 'react-icons/fa';
...
const QuoteList = () => {
  const { quotes, deleteQuote, updateQuote, loading, error, success, replyToQuote } = useQuote(); // Added replyToQuote
  const [editId, setEditId] = useState(null);
  const [editData, setEditData] = useState({});
  const [page, setPage] = useState(1);
  const [viewQuote, setViewQuote] = useState(null);
  ...
  const handleReplySend = async () => {
    ...
    try {
      await replyToQuote(quoteToReply._id, { replyMessage });
      setSuccess2('Reply sent successfully!');
            ...
            {paginatedQuotes.length === 0 && !loading ? (
              ...
              paginatedQuotes.map((quote) => (
                <tr key={quote._id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    {editId === quote._id ? ( // Corrected this line, should be _id, not phone
                      <input
                        name="phone"
                        value={editData.phone}
                        onChange={handleEditChange}
                    ...
                    <div className="flex justify-center space-x-2">
                      {editId === quote._id ? (
                        ...
                        <>
                          ...
                          <button
                            onClick={() => handleReplyClick(quote)} // New Reply button
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full p-6 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <p className="text-sm text-gray-500">Phone</p> {/* Added Phone to View Modal */}
